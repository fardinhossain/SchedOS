/**
 * Educational reference implementations of SJF (non-preemptive).
...
export const sjfCode: Record<SupportedLanguage, string> = {
  c: `#include <stdio.h>

/* Shortest Job First (SJF) — non-preemptive.
   Rule: among all processes that have ARRIVED (arrival <= clock), pick
   the one with the SMALLEST burst time and run it to completion.

   SJF is provably optimal for average waiting time under non-preemptive
   scheduling: putting the shortest remaining job first minimises the
   cumulative wait of every job queued behind it. */
...
/* Return the index of the arrived, unscheduled process with the
   shortest burst.  Tie-break: earlier arrival wins; then lower index
   (i.e. the process that was entered first). */
int select_sjf(Process p[], int n, int clock) {
    int chosen = -1;
    for (int i = 0; i < n; i++) {
...
int main(void) {
  ...
  python: `"""Shortest Job First (SJF) — non-preemptive.
Rule: among all processes that have ARRIVED (arrival <= clock),
pick the one with the SMALLEST burst time and run it to completion.

SJF is provably optimal for average waiting time under non-preemptive
scheduling: serving the shortest job first minimises the cumulative
waiting time of every job queued behind it.
"""
...
def sjf(processes: list[Process]) -> list[tuple[str, int, int]]:
    ...
    while remaining:
        ...
        # SJF selection: smallest burst wins.
...
if __name__ == "__main__":
    ...
    report(demo, sjf(demo))`,

  typescript: `/**
 * Shortest Job First (SJF) — non-preemptive.
 * Rule: among all processes that have ARRIVED, pick the one with the
 * smallest burst time and run it to completion.
 *
 * SJF is provably optimal for average waiting time under non-preemptive
 * scheduling — serving the shortest job first minimises the cumulative
 * waiting time of every job behind it in the ready queue.
 */
...
function sjf(input: ProcessInput[]): SchedulingResult {
  ...
  while (remaining.length > 0) {
    ...
    // SJF selection: shortest burst wins.
