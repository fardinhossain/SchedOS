/**
 * Educational reference implementations of SJF (Shortest Job First - Non-Preemptive).
...
/* Shortest Job First (SJF) — Non-Preemptive
   Rule: Pick available process with shortest burst time, run to completion. */
...
while completed < n:
    ...
    # Pick shortest burst (tie-breaker: earlier arrival)
...
declare -a id at bt ct tat wt rt completed
...
while [ "$completed_count" -lt "$n" ]; do
    ...
    for ((i = 0; i < n; i++)); do
        if [ "\${at[i]}" -le "$current_time" ] && [ "\${completed[i]}" -eq 0 ]; then
            if [ "\${bt[i]}" -lt "$min_bt" ]; then
                min_bt="\${bt[i]}"
                idx="$i"
            elif [ "\${bt[i]}" -eq "$min_bt" ]; then
